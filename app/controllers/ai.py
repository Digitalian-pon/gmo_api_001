...
def determine_trade_action(df, optimized_trade_params):
    ...
    if params.ema_enable:
        ...
        if len(ema_values_1) >= 2 and len(ema_values_2) >= 2:
            ...
            buy_signal = ema_values_1[-2] < ema_values_2[-2] and ema_values_1[-1] >= ema_values_2[
                -1] and ema_diff > 0.05
            sell_signal = ema_values_1[-2] > ema_values_2[-2] and ema_values_1[-1] <= ema_values_2[
                -1] and ema_diff > 0.05
    ...
    if params.rsi_enable:
        rsi = talib.RSI(np.array(df.closes), timeperiod=params.rsi_period)
        if len(rsi) >= 1:
            if rsi[-1] < params.rsi_buy_thread:
                buy_signal = True
                signal_strength += 1
            elif rsi[-1] > params.rsi_sell_thread:
                sell_signal = True
                signal_strength += 1

        logger.debug(f"RSI: {rsi[-1]}")
        logger.debug(f"RSI Buy Signal: {buy_signal}, RSI Sell Signal: {sell_signal}")
