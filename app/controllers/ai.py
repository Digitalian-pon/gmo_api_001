...
from gmocoin.gmocoin import Order
...
class AI(object):
    ...
    def buy(self, candle):
        ...
        try:
            ...
            logger.info(f'action=buy balance={balance.__dict__}')
            ...
            logger.info(f'action=buy status=order_placed trade={trade}')
            could_buy = self.signal_events.buy(self.product_code, candle.time, trade['price'], trade['units'], save=True)
            return could_buy
        except Exception as e:
            logger.error(f'action=buy status=false error={str(e)}')
            return False
    ...
    def sell(self, candle):
        ...
        try:
            ...
            logger.info(f'action=sell open_trades={trades}')
            sum_price = 0
            units = 0
            for trade in trades:
            ...
            logger.info(f'action=sell status=trade_closed sum_price={sum_price}, units={units}')
            could_sell = self.signal_events.sell(self.product_code, candle.time, sum_price/units, units, save=True)
            return could_sell
        except Exception as e:
            logger.error(f'action=sell status=false error={str(e)}')
            return False
        ...
        for i in range(1, len(df.candles)):
            ...
            if params.rsi_enable and rsi_values[i-1] != 0 and rsi_values[i-1] != 100:
                ...
                if rsi_values[i-1] > params.rsi_sell_thread and rsi_values[i] <= params.rsi_sell_thread:
                    sell_point += 1
            if buy_point > 0:
                logger.info('action=trade status=buy_point_detected')
                if not self.buy(df.candles[i]):
                    logger.warning('action=trade status=buy_failed')
                    continue
                self.stop_limit = df.candles[i].close * self.stop_limit_percent
            if sell_point > 0 or self.stop_limit > df.candles[i].close:
                logger.info('action=trade status=sell_point_detected')
                if not self.sell(df.candles[i]):
                    logger.warning('action=trade status=sell_failed')
                    continue
                self.stop_limit = 0.0
